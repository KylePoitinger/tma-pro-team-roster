...
### Data Models
- **ProTeamEntity**: Teams with roster relationship (\	eamId\ PK, name, city, mascot, \List<ProPlayerEntity>\ via OneToMany LAZY fetch)
- **ProPlayerEntity**: Players linked to teams by \	eamName\ string reference (not FK), contains salary, college, physical attributes
- **ProMascotEntity**: Mascots linked to teams by \	eamName\ string reference (not FK), contains \mascotId\ PK, name, description, costume
- **Relationship:** Team.proPlayers and Team.proMascots joined on \	eamName\ string—unusual pattern, preserve when modifying
---
...
- **Console:** http://localhost:8080/h2-console (enabled in \pplication.properties\)
...
- \PUT /players/{playerId}\ – update player (upsert if not found)
...
- \PUT /mascots/{mascotId}\ – update mascot (upsert if not found)
...
Players reference teams via \	eamName\ string field, not foreign key. Updates to team names may orphan player records—be aware when renaming teams.
---
...
3. **String-based FK** – Team-player relationship via \	eamName\ string, not proper foreign key
